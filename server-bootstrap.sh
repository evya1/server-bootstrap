#!/usr/bin/env bash
# Modular Ubuntu server foundation. Run from the extracted release archive.
set -Eeuo pipefail
...
for module in core archive bundle bootstrap/config bootstrap/workspace bootstrap/packages \
    bootstrap/node bootstrap/ai_cli bootstrap/pi bootstrap/uv bootstrap/github_cli bootstrap/ngrok bootstrap/python \
...
on_error() {
    local code=$?
    sb_warn "failed at line ${BASH_LINENO[0]} during '$STEP': ${BASH_COMMAND} (exit $code)"
    {
        echo "STATUS: FAILED"
        echo "step: $STEP"
...
flock -w 1800 9 || sb_die "could not acquire bootstrap lock"
sb_log "server-bootstrap v$BOOTSTRAP_VERSION"
sb_log "log file: $LOG_FILE"
...
STEP=acceptance; bootstrap_acceptance "$ROOT/server-accept.sh"
...
if [[ "$INSTALL_ADDON" == 1 ]]; then
    ...
    case "$ADDON_URL" in
        https://*) : ;;
        file://*|/*) [[ "$ADDON_ALLOW_LOCAL_FILE" == 1 ]] || sb_die "local add-on requires ADDON_ALLOW_LOCAL_FILE=1" ;;
        *) sb_die "ADDON_URL must be HTTPS or a trusted local path" ;;
    esac
    ADDON_ARGS=()
    [[ -z "$ADDON_INSTALL_ARGS" ]] || read -r -a ADDON_ARGS <<< "$ADDON_INSTALL_ARGS"
    sb_install_bundle "$ADDON_NAME" "$ADDON_VERSION" "$ADDON_URL" "$ADDON_SHA256" \
