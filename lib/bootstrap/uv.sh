...
bootstrap_uv() {
    ...
    [[ "$(uname -m)" == x86_64 ]] || { sb_warn "automatic uv install currently supports x86_64 only"; return 0; }
    if [[ -z "$UV_SHA256" ]]; then
        sb_warn "UV_SHA256 is empty; refusing an unverified uv install"
        sb_warn "set UV_VERSION and UV_SHA256, or set INSTALL_UV=0"
        return 0
    fi
    local temp archive
    temp="$(mktemp -d)"; archive="$temp/uv.tar.gz"
    trap 'rm -rf -- "$temp"' RETURN
    sb_log "installing uv $UV_VERSION (pinned and checksum-verified)"
    sb_fetch_verified \
        "https://github.com/astral-sh/uv/releases/download/$UV_VERSION/uv-x86_64-unknown-linux-gnu.tar.gz" \
        "$UV_SHA256" "$archive" || return
    sb_extract_archive "$archive" "$temp/extracted"
    local binary
