...
bootstrap_acceptance() {
    ...
    [[ -x "$accept_script" ]] || { gsb_warn "gpu acceptance script not found: $accept_script"; return 0; }
    ...
    case "$result" in
        ...
        *) gsb_warn "gpu acceptance script failed with exit $result"; return "$result" ;;
    esac
    if [[ "$RUN_ACCEPT_TEST" == strict && "$result" != 0 ]]; then
        gsb_die "GPU acceptance policy is strict; verdict=$ACCEPT_RESULT"
    fi
}
