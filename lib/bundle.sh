...
gsb_bundle_validate_name() {
    [[ "$1" =~ ^[A-Za-z0-9._-]+$ ]] || gsb_die "invalid bundle name: $1"
}

gsb_bundle_materialize() {
    ...
    if [[ "$source_ref" == https://* ]]; then
        case "$source_ref" in
            ...
            *) gsb_die "remote bundle URL must end with .tar.gz, .tgz, .tar.xz, .txz, or .zip" || return ;;
        esac
        gsb_fetch_verified "$source_ref" "$expected" "$output" || return
    else
        local_path="${source_ref#file://}"
        [[ -f "$local_path" ]] || gsb_die "bundle archive not found: $local_path" || return
        gsb_verify_file "$local_path" "$expected" || return
        output="$(realpath -m -- "$local_path")"
    fi
    printf -v "$output_var" '%s' "$output"
}

gsb_bundle_write_state() {
...
gsb_install_bundle() (
    ...
    gsb_bundle_validate_name "$name" || return
    [[ -n "$version" ]] || gsb_die "bundle version is required" || return
    [[ -n "$source_ref" ]] || gsb_die "bundle source is required" || return
    gsb_valid_sha256 "$expected" || gsb_die "bundle SHA-256 must be 64 hexadecimal characters" || return
    [[ "$installer" != */* ]] || gsb_die "installer must be a filename, not a path: $installer" || return

    temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/gpu-bundle.XXXXXX")"
    trap 'rm -rf -- "$temp_dir"' EXIT
    gsb_bundle_materialize "$source_ref" "$expected" "$temp_dir" archive || return
    actual="$(gsb_sha256 "$archive")"
    ...
    if [[ -z "$current_version" && -f "$legacy_dir/version" ]]; then
        ...
        current_sha="$actual"
        gsb_log "migrating legacy add-on state for $name"
        gsb_bundle_write_state "$state_dir" "$current_version" "$current_sha" "$source_ref" "$installer"
    fi

    if [[ "$force" != 1 && "$current_version" == "$version" ]]; then
        if [[ -n "$current_sha" && "${current_sha,,}" != "${actual,,}" ]]; then
            gsb_die "$name $version is already recorded with a different archive hash; use --force only after review"
            return 1
        fi
        gsb_log "bundle already installed: $name $version (skipped)"
        [[ "$delete_after" == 1 ]] && gsb_bundle_delete_local_source "$source_ref" "$checksum_sidecar"
        return 0
    fi

    extract_dir="$temp_dir/extracted"
    gsb_extract_archive "$archive" "$extract_dir" || return
    mapfile -d '' matches < <(find "$extract_dir" -type f -name "$installer" -print0)
    (( ${#matches[@]} == 1 )) \
        || gsb_die "expected exactly one '$installer' in bundle, found ${#matches[@]}" || return
    ...
    if [[ -f "$install_root/VERSION" ]]; then
        ...
        [[ "$shipped_version" == "$version" ]] \
            || gsb_die "bundle VERSION=$shipped_version does not match requested version=$version" || return
    fi

    gsb_log "installing bundle: $name $version"
    ( cd -- "$install_root" && bash "$install_file" "${installer_args[@]}" )
    gsb_bundle_write_state "$state_dir" "$version" "$actual" "$source_ref" "$installer"
    gsb_log "bundle installed: $name $version"
    [[ "$delete_after" == 1 ]] && gsb_bundle_delete_local_source "$source_ref" "$checksum_sidecar"
    return 0
)
