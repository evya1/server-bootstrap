...
ml_fixture_lock() {  # out, backend, arch, cuda, index, [local version suffix], [extra name=version...]
    ...
    shift 6 || shift $#
    {
        ...
        printf '%s\n' '--index-url https://pypi.org/simple' "--extra-index-url $index" ''
...
printf '%s\n' 'cpu none x86_64,aarch64 https://download.pytorch.org/whl/cpu' \
    'cu130 13.0 x86_64 https://download.pytorch.org/whl/cu130' > "$MLV/backends.txt"
...
if ml_generate "$pinned_uv" "$MLG/body.txt" && [[ -f "$MLG/profile/locks/cpu-x86_64.txt" ]]; then
    ...
    (( gen_missing == 0 )) && ok "ml-lock resolves for Python 3.12 with hashes, from the official index, without builds"
...
case "$1" in
    ...
    pip)
        ...
        while (( $# )); do case "$1" in --python) python="$2"; shift 2 ;; --*) shift ;; *) lock="$1"; shift ;; esac; done
...
: > "$MLI/uv.log"
if ml_install --backend auto; then ok "first install succeeds"; else bad "first install: $(cat "$MLI/out")"; fi
...
grep -q -- "^venv --python $ML_PY312 --no-python-downloads" "$MLI/uv.log" \
    && grep -q -- '^pip sync .*--require-hashes --no-build .*'"$MLI/locks/cpu-$ML_ARCH.txt"'$' "$MLI/uv.log" \
    && ok "the environment is built from Python 3.12 and synced from the lock with hashes required" \
